...
from pychange.preprocess import create_summary_stats
from pychange.segment import binary_segmentation
from pychange.costs import ParametricCost


if __name__ == '__main__':
    # Creating test series
    x = np.concatenate([np.random.normal(-1, 1, (100000,)),
                        np.random.normal(1, 2, (100000,)),
                        np.random.normal(-1, 1, (100000,)),
                        np.random.normal(-1, 1, (100000,)),
                        np.random.normal(-1, 1, (100000,)),
                        np.random.normal(-1, 1, (100000,)),
                        np.random.normal(-1, 1, (100000,)),
    ...
    jit_binary_segmentation = njit(fastmath=True)(binary_segmentation)
    _ = jit_binary_segmentation(x, 30, 100, 100, jit_sum_stats, jit_normal_mean_var_cost)
    ...
    for k, v in test_dict.items():
        start_time = time()
        #for _ in range(1000):
        _ = binary_segmentation(x, 30, 100, 100, jit_sum_stats, v)
        print(f'{k}: {(time() - start_time)}')
    ...
    print('\nRuptures Binseg Timing')
    for k, v in test_dict.items():
        start_time = time()
        algo = rpt.Binseg(custom_cost=ParametricCost(cost_fn=v)).fit(x)
