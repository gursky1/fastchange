...
if __name__ == '__main__':
    ...
    for seg_py, seg_r in [('PeltSeg', 'PELT')]:
        #for dist, dist_fn in [('Normal', normal_data), ('Poisson', poisson_data), ('Exponential', exp_data), ('Gamma', gamma_data), ('Empirical', normal_data)]:
        for dist, dist_fn, n_list in [('Normal', normal_data, map(int, [1e2, 5e2, 1e3, 5e3, 1e4, 5e4, 1e5])), ('Empirical', normal_data, map(int, [1e2, 5e2, 1e3, 5e3, 1e4]))]:
            #for stat in ['Mean', 'Var', 'MeanVar']:
            for stat in ['Mean', 'MeanVar']:
                ...
                for pen in ['MBIC']:
                    ...
                    for n in n_list:
                        try:
                            ...
                            if dist == 'Empirical':
                                py_times = timeit.repeat(f'{seg_py}({dist}Cost(k=10), {pen.lower()}_penalty, min_len=10, max_cps=100).fit(data).predict()', repeat=repeats, number=1, globals=globals()) 
                                ...
                                py_times = timeit.repeat(f'{seg_py}({dist}{stat}Cost(), {pen.lower()}_penalty, min_len=10, max_cps=100).fit(data).predict()', repeat=repeats, number=1, globals=globals()) 
                                r_times = timeit.repeat(f"ROfflineChangepoint('{stat.lower()}', penalty='{pen}', method='{seg_r}', test_stat='{dist}', minseglen=10, Q=100).fit(data).predict()", repeat=repeats, number=1, globals=globals())

                            py_mean = np.mean(py_times)
                            r_mean = np.mean(r_times)

                            with open('synthetic_profile.csv', 'a') as f:
                                f.write(f'{n},{seg_py},{dist},{stat},{pen},{py_mean},{r_mean}\n')
                            print(n, seg_py, dist, stat, (r_mean / py_mean).round(4))
